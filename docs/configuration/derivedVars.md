...
- **`id`** *(string, required)*: Code by which to refer to the variable e.g. `tas`, `SPI`, `pr` etc. Items must be unique.
- **`inputVars`** *(string, required)*: Comma-separated list of input variables to be used in generating this new variable.
- **`outputVars`** *(string, required)*: Comma-separated list of output variables generated by the script. Some functions generate more than one variable at a time e.g. FWI and this is where they should be listed. Currently limited to one but will be extended in the future - issue #92 https://github.com/Klimaatlas/KAPy/issues/92.

...
- **`variables`**: List of input variables required to calculate the indicator. Must be at least one specified.
  - **One of**
    ...
    - *array*
      - **Items** *(string)*
- **`datasets`** *(string)*: Comma-separated list of dataset IDs for which the indicator is to be calculated. IDs should match those defined as primary (input), secondary, calibration or tertiary variables. In addition, `all` selects all datasets.
...
- **`statistic`** *(string, required)*: Statistic to be used to calculate the indicator. KAPy has five basic types of indicators built in. 1) `mean` calculates an average across the period. 2)`count` calculates the average number of days per year in the period above or below a threshold. Additional parameters must be supplied via the `additionalArgs` field: `threshold` is the threshold value, and `op` is the comparison operator - one of `>`,`gt`,`<`,`lt`,  `>=`, `ge`, `<=` or `le`.  3) `annmax` calculates the maximum value of the variable over the season or year - in cases where there is period binning enabled, the average annual maximum is calculated. 4) `annmin` calculates the average minimum in the same manner as `annmax` and  5) `custom` calls the function defined in `customScriptPath` and `customScriptFunction` and utilised arguments supplied as a dictionary in `additionalArgs`. Must be one of: `["custom", "mean", "count", "annmax", "annmin"]`.
